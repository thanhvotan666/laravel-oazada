                    ...
                    <div class="p-4">
                        <div class="border rounded-4 bg-white p-3 d-flex flex-column gap-3">
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <div class="fs-6  fw-semibold  text-danger">-${{ $order->discount_value ?? 0 }}</div>
                            </div>
                            <div class="d-flex justify-content-between ">
                                <div>Subtotal :</div>
                        ...
                        <div class="border rounded-4 bg-white p-3 d-flex flex-column gap-3 mt-3">
                            ...
                            <form action="{{ route('supplier.orders.update', ['order' => $order->id]) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <input type="hidden" name="cause" id="cause" value="" disabled>
                                ...
                                @endswitch
                                    "
                                    @disabled($order->status == 'canceled' || $order->status == 'shipped')>
                                    ...
                                    <option @selected($order->status === 'shipping') value="shipping">shipping</option>
                            ...
                            @if ($order->status == 'canceled')
                                <div>
                                    <div>Cause :</div>
                                    <div>{{ $order->cancel->cause }}</div>
                                </div>
                            @endif
                            <script>
                                function changeStatus(element) {
                                    if (element.value === 'canceled') {
                                        const cause = prompt("Please enter your cause:", "Supplier canceled!");
                                        document.getElementById('cause').disabled = false;
                                        document.getElementById('cause').value = cause;
                                        console.log(document.getElementById('cause').disabled, document.getElementById('cause').value);
                                    }
                                    element.form.submit();
                                }
                            </script>
                            ...
                            <div>
                                <table class="table table-hover ">
                                    ...
                                    <tbody>
                                        <tr>
                                            <th scope="row"></th>
                                            <td><a class="@if ($productVariant) text-success fw-semibold @else text-danger @endif "
                                                    @if ($productVariant) href="{{ route('product', ['id' => $productVariant->product->id]) }}" @endif>
                                                    {{ $order->detail->name }}
                                                </a>
                                            </td>
                                            <td>{{ $order->detail->variant }}</td>
                                            <td>{{ $order->detail->price }}</td>
                                            <td class="text-center">{{ $order->detail->quantity }}</td>
                                            <td>${{ $order->detail->quantity * $order->detail->price }}</td>
                                            <td></td>
                                        </tr>
                                </table>
                            </div>
