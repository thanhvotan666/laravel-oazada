...
@section('content')
    ...
    <form method="POST" action="{{ route('supplier.products.store') }}" class="container-fluid d-flex flex-column gap-4"
        enctype="multipart/form-data">
        @csrf
        @php
            //create
            $inputType = old('type', '');
            //$inputType = 'color;size';
            //create
            $inputVariantsData = old('value', '');
            //$inputVariantsData = 'red, l:1234, 0, 0.1;blue, l:1, 0, 0.1;blue, xl:12, 0, 0.1;red, xl:1234, 0, 0.13';
            //create
            $keyAttributes = old('key_attributes', '');
            //$keyAttributes = '1[:]asdsads[;]2[:]jhijhvn[;]3[:]5327t6fg';
        @endphp
        {{-- create --}}
        <input type="hidden" id="hidden-type" name="type" value="{{ $inputType }}">
        {{-- create --}}
        <input type="hidden" id="hidden-value" name="value" value="{{ $inputVariantsData }}">
        <div class="">
            <div class="h2 fw-bold">Add a product</div>
            ...
            <div class="col-xl-8 ">
                <div class="p-5 w-100 d-flex flex-column gap-4">
                    <div class="">
                        ...
                        <input class="form-control" type="text" name="name" id="name" {{-- create --}}
                    ...
                    <div class="container mt-5">
                        ...
                        <div class="image-upload">
                            ...
                            <img id="previewImage" src="{{ asset('storage/image/icon/image.svg') }}" alt="Preview"
                                class="rounded" width="150" height="150"
                                onclick="document.getElementById('fileUpload').click(); return false;">
                        </div>
                    </div>
                    <div class="">
                                ...
                                <div class="w-100 d-flex flex-column justify-content-around p-4 gap-4">
                                    ...
                                    <div>
                                        <select id="key_attribute_select" class="select-control form-control w-50"
                                            ...
                                            @if ($keyAttributes != '')
                                                @php
                                                    $arrayKeyAttributes = array_map(function ($keyAtt) {
                                                        $arrKeyAtt = explode('[:]', $keyAtt);
                                                        return [
                                                            'name' => $arrKeyAtt[0],
                                                            'value' => $arrKeyAtt[1],
                                                        ];
                                                    }, explode('[;]', old('key_attributes', $keyAttributes)));
                                                @endphp
                                                @foreach ($arrayKeyAttributes as $arrayKeyAttribute)
                                                    <option value="{{ $arrayKeyAttribute['value'] }}">
                                                        {{ $arrayKeyAttribute['name'] }}
                                                    </option>
                                                @endforeach
                                            @endif
                            ...
                            <div>
                                <select id="category_type" name="category_type_id"
                                    ...
                                    @foreach ($category_types as $category_type)
                                        <option value="{{ $category_type->id }}" @selected(old('category_type_id', '') === $category_type->id)>
                                            {{ $category_type->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                <select name="category_id" id="ajaxCategory" class="form-select text-secondary small"
                                    ...
                                    @php
                                        ...
                                        do {
                                            $i--;
                                            if (old('category_type_id') ?? '' === $category_types[$i]->id) {
                                                break;
                                            }
                                        } while ($i >= 1);
                                    @endphp
